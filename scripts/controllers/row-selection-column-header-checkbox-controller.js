define(['app'], function (app) {
    app.controller('rowSelectionColumnHeaderCheckBoxController', [
            '$scope', 'rowSelectionColumnCheckBoxService', '$log',
            function ($scope, rowSelectionColumnCheckBoxService, $log) {

                //rowSelectionColumnCheckBoxService.resetRowSelection();
                $scope.viewModel = rowSelectionColumnCheckBoxService.dataModel;

                $scope.selectAll = function () {
                    //todo - need to add an enum
                    if ($scope.viewModel.isMasterBoxChecked === false) {
                        $scope.viewModel.isMasterBoxChecked = true;
                        rowSelectionColumnCheckBoxService.checkedAll();
                    } else {
                        $scope.viewModel.isMasterBoxChecked = false;
                        rowSelectionColumnCheckBoxService.clearRowStatus();
                    }

                    if (angular.isFunction($scope.genericGridConfig.onRowSelect)) {
                        var selectedIdList = rowSelectionColumnCheckBoxService.getSavedIdList();
                        $log.info('selected id list : ' + selectedIdList.length);
                        $scope.genericGridConfig.onRowSelect(null, null, selectedIdList);
                    }
                };
            }
    ])
        .controller('rowSelectionColumnCellCheckBoxController', [
            '$scope', 'rowSelectionColumnCheckBoxService',
            function ($scope, rowSelectionColumnCheckBoxService) {
                var rowId = $scope.data.id;

                $scope.viewModel = rowSelectionColumnCheckBoxService.dataModel;
                $scope.style = $scope.genericGridConfig.getStyleInfo($scope.data);
                $scope.rowStatus = rowSelectionColumnCheckBoxService.getRowStatus(rowId);

                $scope.singleRowSelectClicked = function () {
                    $scope.rowStatus.isChecked = !$scope.rowStatus.isChecked;

                    if ($scope.rowStatus.isChecked) {
                        rowSelectionColumnCheckBoxService.incrementCounter(rowId);
                    } else {
                        rowSelectionColumnCheckBoxService.decrementCounter(rowId);
                    }
                ...
                if (rowSelectionColumnCheckBoxService.isRowIdSelected(rowId)) {
                    $scope.rowStatus.isChecked = true;
                }

                function checkAllItemsAreSelected() {
                    rowSelectionColumnCheckBoxService.setMasterCheckBoxState();
                }

                checkAllItemsAreSelected();
            }
        ])
        .service('rowSelectionColumnCheckBoxService', [
            '$log', 'genericGridCommonService', '$timeout', function ($log, genericGridCommonService, $timeout) {
